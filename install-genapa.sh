...
INSTALLER=$(find "$EXTRACT_DIR" -maxdepth 2 -name 'install-genapa-forge.sh' | head -1)
if [[ -z "$INSTALLER" ]]; then
  echo "  [X] install-genapa-forge.sh not found in bundle." >&2
  exit 1
fi
