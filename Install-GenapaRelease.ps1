<#
.SYNOPSIS
    GENAPA Forge release bootstrapper. Downloads and runs the platform-specific installer from GitHub Releases.
.DESCRIPTION
...
.EXAMPLE
    .\Install-GenapaRelease.ps1 -Version v1.2.0 -Slug test
#>
...
param(
    ...
    [switch]$Force,
    [switch]$ShowLogs,
    [Nullable[bool]]$EnableAutostart = $null,
    [switch]$AllowNewSecretsWithExistingVolume,
    [switch]$PreRelease,
    [switch]$DownloadOnly,
...
# PowerShell 5.1 compatibility: $IsWindows, $IsLinux, $IsMacOS are only defined in PS 6+.
if (-not (Get-Variable IsWindows -ValueOnly -ErrorAction Ignore)) {
...
function Assert-PowerShellCompatible {
    if (-not $IsWindows) {
        return
    }

    if ($PSVersionTable.PSVersion.Major -lt 5) {
        Write-Fail "Windows PowerShell 5.1 or newer is required. Current version: $($PSVersionTable.PSVersion)"
        exit 1
    }
}
...
# Locate the platform bundle asset
...
if ($detectedOs -eq 'windows') {
    $installer = Get-ChildItem -Path $extractDir -Filter 'Install-GenapaBundle.ps1' -Recurse | Select-Object -First 1
} else {
    $installer = Get-ChildItem -Path $extractDir -Filter 'install-genapa-bundle.sh' -Recurse | Select-Object -First 1
}
...
if ($DownloadOnly) {
    ...
    if ($installer.Name -match '\.sh$') {
        Write-Info "Run manually:  bash `"$($installer.FullName)`" --slug $Slug"
    }
    else {
        Write-Info "Run manually:  powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$($installer.FullName)`" -Slug $Slug"
    }
    exit 0
}
...
if ($ShowLogs) {
    $installerArgs['ShowLogs'] = $true
}

if ($null -ne $EnableAutostart) {
    $installerArgs['EnableAutostart'] = $EnableAutostart
}

if ($AllowNewSecretsWithExistingVolume) {
    $installerArgs['AllowNewSecretsWithExistingVolume'] = $true
}
...
if ($installer.Name -match '\.sh$') {
    ...
    $bashInstallerArguments = @('--slug', $Slug)
    if ($Force) {
        $bashInstallerArguments += '--force'
    }

    if ($ShowLogs) {
        $bashInstallerArguments += '--show-logs'
    }

    if ($null -ne $EnableAutostart) {
        $bashInstallerArguments += if ([bool]$EnableAutostart) { '--enable-autostart' } else { '--no-autostart' }
    }

    if ($AllowNewSecretsWithExistingVolume) {
        $bashInstallerArguments += '--allow-new-secrets-with-existing-volume'
    }

    & bash $installer.FullName @bashInstallerArguments
} else {
